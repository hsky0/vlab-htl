...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    inccrect_list = []
    def protect_withdraw(amount, pw):
        nonlocal inccrect_list, balance
        if len(inccrect_list) >= 3:
            return "Too many incorrect attempts. Attempts: {0}".format(inccrect_list)
        else:
            if pw == password:
                if balance < amount:
                    return 'Insufficient funds'
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    if type(check) == str:
        return check            # is check is string, it indicates the old_pass is not correct
    def helper(amount, passwd):
        if passwd == old_pass or passwd == new_pass:
