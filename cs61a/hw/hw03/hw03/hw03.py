...
def g_iter(n):
    ...
    if n <= 3:
        ...
        k = 3
        while k < n:
            first, mid, last = mid, last, 1 * last + 2 * mid + 3 * first
            k += 1
        return last
...
def missing_digits(n):
    ...
    def helper(n, count):
        if n < 10:
            ...
            return helper(n // 10, count + (n % 10 - (n // 10) % 10 - 1))
    return helper(n, 0)
...
def make_anonymous_factorial():
    ...
    return (lambda f : lambda x: f(f, x))(lambda f, n: 1 if n == 0 else mul(n, f(f, sub(n, 1))))

    return (lambda f: lambda x: f(f, x))(lambda f, n: 1 if n == 0 else mul(f, f(f, sub(n, 1))))