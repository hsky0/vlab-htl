...
def inc_subseqs(s):
    ...
    """
    """This problem is very difficult, I can't understand it!"""
...
def reverse(lst):
    ...
    mid = len(lst) // 2
    left, right, index = mid, mid, 0
    while index < len(lst):
        if mid % 2 == 0:
            lst[right], lst[left - 1] = lst[left - 1], lst[right]
            right, left = right + 1, left - 1
            index = right
        else:
            lst[right + 1], lst[left - 1] = lst[left - 1], lst[right + 1]
            right, left = right + 1, left - 1
            index = right + 1
...
def make_glookup(class_assignments):
    ...
    """
    "*** YOUR CODE HERE ***"
    total_point_sofar, std_point_sofar = 0, class_assignments['Extra credit']
    def cal(messages, point):
        nonlocal total_point_sofar, std_point_sofar
        for key in class_assignments:
            assert key in class_assignments, 'Invalid key name'
            if key == messages:
                total_point_sofar += class_assignments[key]
                std_point_sofar += point
                return std_point_sofar / total_point_sofar
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n <= 2:
        return 1
    return num_trees(n - 1) * 2 * (2 * n - 3) // n  # From PKU, need to derivate
