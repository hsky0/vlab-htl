...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            a = subseq_helper(s[1:], prev)  # the remaind elements need to compare with prev
            b = subseq_helper(s[1:], s[0])  # compared to the previous element with the latter element
...
def reverse(lst):
    ...
    "*** YOUR CODE HERE ***"
    mid = len(lst) // 2
    index, lst_size = 0, len(lst)
    while index < mid:
        lst[index], lst[lst_size - index - 1] = lst[lst_size - index - 1], lst[index]
        index += 1
...
def make_glookup(class_assignments):
    ...
    """
    "*** YOUR CODE HERE ***"
    std_total_point = class_assignments['Extra credit']
    total_point = 0
    def helper(messages, point):
        nonlocal total_point, std_total_point
        total_point += class_assignments[messages]
        std_total_point += point
        return std_total_point / total_point
    return helper
