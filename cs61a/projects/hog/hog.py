...
def roll_dice(num_rolls, dice=six_sided):
    ...
    "The follow codes have a bit of error, can pass the test"
    # total_score, i = 0, 1
    # while i <= num_rolls:
    #     curr_socre = dice()
    #     if curr_socre == 1:
    #         return 1
    #     else:
    #         total_score += curr_socre
    #     i += 1
    
    # return total_score

    "From PKU"
    all_score = [dice() for i in range(num_rolls)]
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            this_turn_score = take_turn(roll_num, score1, dice)
            score0 += this_turn_score
            if feral_hogs and abs(last_turn_score0 - roll_num) == 2:
                score0 += 3
            ...
            this_turn_score = take_turn(roll_num, score0, dice)
            score1 += this_turn_score
            if feral_hogs and abs(last_turn_score1 - roll_num) == 2:
                score1 += 3
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            ...
            if gain > running_high:
                ...
                return announce_highest(who, score0, gain)      # This return value is important
    ...
    def say(*scores):
        ...
        if gain > running_high:
            print(gain, "point(s)! That's the boggest gain yet for Player ", who)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    average_scores = []
    for i in range(10):
        average_scores.append(make_averaged(roll_dice, trials_count)(i + 1, dice)) # this idea from PKU
    
    index , maxVal= 0, 0.0
    for i in range(10):
        if maxVal < average_scores[i]:
            maxVal = average_scores[i]
            index = i
    return index + 1
    # END PROBLEM 9
    """From PKU"""
    ma = make_averaged(roll_dice, trials_count)     # this is a function
...
def swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    #return 6  # Replace this statement
    temp_score = score + free_bacon(opponent_score)
    if is_swap(temp_score, opponent_score):
        if temp_score < opponent_score:
            return 0
        else:
            return num_rolls
    else:
